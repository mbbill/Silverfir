// source code is from the wasm32-wasi-benchmark project
// https://github.com/second-state/wasm32-wasi-benchmark.git
...
static void do_job(int minimum_Tree_Depth, int maximum_Tree_Depth) {
        ...
        tree_Checksums[(maximum_Tree_Depth - minimum_Tree_Depth + 2) / 2];
    {
        ...
        for (uint32_t tree_Depth = minimum_Tree_Depth;
             tree_Depth <= maximum_Tree_Depth;
             tree_Depth += 2) {
            uint32_t total_Trees_Checksum = 0;
            for (uint32_t iterations = 1 << (maximum_Tree_Depth - tree_Depth + minimum_Tree_Depth);
                 iterations-- > 0;) {
            ...
            tree_Checksums[(tree_Depth - minimum_Tree_Depth) / 2] = total_Trees_Checksum;
        }
    ...
    for (uint32_t tree_Depth = minimum_Tree_Depth;
         tree_Depth <= maximum_Tree_Depth;
         tree_Depth += 2)
        printf("%" PRIu32 "\t trees of depth %" PRIu32 "\t check: %" PRIu32 "\n", 1 << (maximum_Tree_Depth - tree_Depth + minimum_Tree_Depth), tree_Depth, tree_Checksums[(tree_Depth - minimum_Tree_Depth) / 2]);
...
int main(int argc, char * argv[]) {
    const uint32_t minimum_Tree_Depth = 4,
                   maximum_Tree_Depth = atoi(argv[1]) < minimum_Tree_Depth + 2
                                            ...
                                            : atoi(argv[1]);
    CHECK_TIME(do_job(minimum_Tree_Depth, maximum_Tree_Depth));
    return 0;
}
