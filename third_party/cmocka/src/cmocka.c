...
static void fail_if_leftover_values(const char * test_name) {
    ...
    if (check_for_leftover_values_list(&global_call_ordering_head, "%s function was expected to be called but was not.\n")) {
        error_occurred = 1;
    }
    if (error_occurred) {
        exit_test(1);
 ...
 */
static void add_symbol_value(ListNode * const symbol_map_head, const char * const symbol_names[], const size_t number_of_symbol_names, const void * value, const int refcount) {
    const char * symbol_name;
...
static int float_compare(const float left, const float right, const float epsilon) {
    float absLeft;
    float absRight;
    float largest;
    float relDiff;
...
static int float_values_equal_display_error(const float left, const float right, const float epsilon) {
    const int equal = float_compare(left, right, epsilon);
    if (!equal) {
        cm_print_error(FloatPrintfFormat " != " FloatPrintfFormat "\n", left, right);
    }
    return equal;
}
 ...
 * an error is displayed and 0 is returned. */
static int float_values_not_equal_display_error(const float left, const float right, const float epsilon) {
...
static void expect_set(
    ...
    _expect_check(
        function, parameter, file, line, check_function, check_data.value, &check_integer_set->event, count);
}
...
void _assert_float_equal(const float a, const float b, const float epsilon, const char * const file, const int line) {
    if (!float_values_equal_display_error(a, b, epsilon)) {
        _fail(file, line);
    }
}

void _assert_float_not_equal(const float a, const float b, const float epsilon, const char * const file, const int line) {
...
static void vcm_print_error(const char * const format, va_list args) CMOCKA_PRINTF_ATTRIBUTE(1, 0);
...
void * _test_malloc(const size_t size, const char * file, const int line) {
    ...
    allocate_size = size + (MALLOC_GUARD_SIZE * 2) + sizeof(struct MallocBlockInfoData) + MALLOC_ALIGNMENT;
    ...
    ptr = (char *)(((size_t)block + MALLOC_GUARD_SIZE + sizeof(struct MallocBlockInfoData) + MALLOC_ALIGNMENT) & ~(MALLOC_ALIGNMENT - 1));
    ...
    block_info.ptr = ptr - (MALLOC_GUARD_SIZE + sizeof(struct MallocBlockInfoData));
...
void _test_free(void * const ptr, const char * file, const int line) {
    ...
    block_info.ptr = block - (MALLOC_GUARD_SIZE + sizeof(struct MallocBlockInfoData));
    /* Check the guard blocks. */
    {
        char * guards[2] = {block - MALLOC_GUARD_SIZE, block + block_info.data->size};
...
static size_t display_allocated_blocks(const ListNode * const check_point) {
    ...
    for (node = check_point->next; node != head; node = node->next) {
        ...
        cm_print_error(SOURCE_LOCATION_FORMAT ": note: block %p allocated here\n", block_info.data->location.file, block_info.data->location.line, block_info.data->block);
        allocated_blocks++;
    }
...
static void free_allocated_blocks(const ListNode * const check_point) {
    ...
    while (node != head) {
        ...
        node = node->next;
        free(discard_const_p(char, block_info.data) + sizeof(struct MallocBlockInfoData) + MALLOC_GUARD_SIZE);
    }
}
...
void cmprintf(enum cm_printf_type type, size_t test_number, const char * test_name, const char * error_message) {
    enum cm_message_output output;
    ...
    if (rc == 0) {
        ...
        for (i = 0; i < total_tests; i++) {
            ...
            if (rc == 0) {
                ...
                snprintf(err_msg, sizeof(err_msg), "Could not run test: %s", cmtest->error_message);
...
int _run_group_tests(const UnitTest * const tests, const size_t number_of_tests) {
    ...
    failed_names = (const char **)malloc(number_of_tests * sizeof(*failed_names));
